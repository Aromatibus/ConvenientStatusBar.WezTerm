...
local function fetch_weather_data(config)
    ...
    -- 現在 (index 1), 3時間後 (index 2), 24時間後 (index 9)
    if #entries >= 1 then
...
function M.setup(opts)
    -- 置換ミスを防ぐため変数の間にスペースを入れたデフォルトフォーマット
    local def_fmt =
        " $user_ic $user " ..
    ...
    wezterm.on('update-right-status', function(window, pane)
        ...
        local use_weather = fmt_lower:find("$weather") or fmt_lower:find("$temp") or fmt_lower:find("$city")
        ...
        local replace_map = {
            ...
            ["$time24"] = wezterm.strftime('%H:%M'), ["$loc_ic"] = has_weather_api and "" or "",
